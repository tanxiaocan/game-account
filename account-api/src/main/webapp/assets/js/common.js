...
var comJs = {
	 ...
	 * @returns {String}
	 */
	appendOpForUser : function() {
		var buttonText = '<a class="btn btn-default" onclick="userJs.updateStatus($(this))" role="button">修改状态</a>';
		var opTd = '<td>'+buttonText+"</td>";
		return opTd;
	},
	/**
	 * 添加下拉操作按钮 for group_query
	 * 
	 * @returns {String}
	 */
	appendOpTdForBattle : function() {
		var buttonText = '<a class="btn btn-default" onclick="battleJs.openSession($(this))" role="button">继续此战</a>';
		var opTd = '<td>'+buttonText+"</td>";
		return opTd;
	},
	/**
...
//					comJs.getPage(data.data);
							...
							ths
									.each(function() {
										...
										if($(this).attr("otherParms")){
											...
											var parmFields = parmaName.split(".");
											var ParmFieldVal;

											if (parmFields.length == 1) {
												ParmFieldVal = value[parmaName];
										...
										if (fieldName == 'order') {
											tr += "<td>" + (n + 1) + "</td>";
										} else if (fieldName == 'operation-battle') {
											tr += comJs.appendOpTdForBattle();
										} else if (fieldName == 'operation-user') {
											tr += comJs.appendOpForUser();
										} else {
											tr += "<td>" + fieldVal + "</td>";
										}
									});
	...
	getPage : function(pageInfo) {
		...
		var pageTagTop = '';
		pageTagTop += '<div class="panel panel-default" id="com_list_msg"><div class="bootstrap-admin-panel-content">';
		pageTagTop += '<nav class="pull-right"><span>当前第' + pageIndex + '页，共'
				+ totalPageCount + '页,共' + total + '条</span> </nav>';
		pageTagTop += '</div></div>';
		tb.parent().parent().before(pageTagTop);
		// 构建分页标签
		var pageTag = '';
		pageTag += '<nav class="pull-right" id="com_list_nav"><ul class="pagination">';
		 ...
		 * 
		 * var pageIndex_loc = pageIndex; for (var i = 0; i < 10; i++) { pageLi += '
		 * <li id="li'+(i+1)+'"><a href="getClickPage(' + pageIndex_loc + ')">' +
		 * pageIndex_loc + '</a></li>'; pageIndex_loc++; } } else { for (var
		 * i = 0; i < totalPageCount; i++) { pageLi += '
		 * <li id="li'+(i+1)+'"><a href="getClickPage(' + (i + 1) + ')">' + (i +
		 * 1) + '</a></li>'; } }
		 */
		pageTag += pageLi;
